...
export async function getAllLocationInfo() {
  ...
  locationInfoPromise = (async () => {
    ...
    const culturalSpaceData = readFile(
      path.join(process.cwd(), 'data', 'culturalSpace.json'),
      'utf-8'
    );
    ...
    cachedLocationInfo = {
      localInfoData,
      parkInfo: parkJson,
      dodreamgilInfo: dodreamgilJson,
      culturalSpaceInfo: culturalSpaceJson,
    };
  ...
  try {
    return await locationInfoPromise;
  } finally {
    locationInfoPromise = null;
  }
}

// location(한국어)은 로직/필터 키로 유지. location_en은 표시 전용(getDistrictLabel).
